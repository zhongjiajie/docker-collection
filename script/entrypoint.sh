...
case "$1" in
  namesrv)
    ...
    sed -i "s/-server -Xms4g -Xmx4g -Xmn2g -XX:PermSize=128m -XX:MaxPermSize=320m/-server ${JVM_MEMORY}/g" runserver.sh
    exec sh mqnamesrv
    ;;
  broker)
    ...
    sed -i "s/-server -Xms8g -Xmx8g -Xmn4g/-server ${JVM_MEMORY}/g" runbroker.sh
